...
while IFS=";" read -r title content ticketId;do
    if [ "$ticketId" == "$ticketIdalt" ]; then
        code+="<div><h2>$title</h2><p>$content</p></div><div><form action='https://informatik.hs-bremerhaven.de/docker-step2023-team-06-web/cgi-bin/swe1-project/ticket-edit/edit-ticket-backlog.sh' method='get'><input type='hidden' name='ticketId' value=$ticketId><input type='text' name='title' id='title' value=$title><br><input type='text' name='content' value=$content> <br><input type='submit' value='Ticket bearbeiten'></form></div><div><form action='https://informatik.hs-bremerhaven.de/docker-step2023-team-06-web/cgi-bin/swe1-project/delete-ticket/delete-backlog.sh' method='get'><input type='hidden' name='ticketId' value=$ticketId><input type='submit' value='Ticket löschen'></form></div>"
    fi
